  ...
  private def renderPrimitive[T](prim: Primitive.Aux[T]): T => Line =
    ...
    (prim match {
      ...
      case Primitive.Timestamp =>
        (ts: java.time.Instant) => line"$timestamp_(${ts.getEpochSecond()}L, ${ts.getNano()})"
      case Primitive.LocalDate =>
        (date: java.time.LocalDate) => line"$localdate_(${date.toEpochDay()})"
      case Primitive.LocalTime =>
        (time: java.time.LocalTime) => line"$localtime_(${time.toSecondOfDay()}, ${time.getNano()})"
